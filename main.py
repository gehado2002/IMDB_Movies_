...
st.markdown(
    ...
    <div class="top-right-gif">
        <img src="https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExOG1zM2JjZ3UyY25wcTdlZzc3aG16MWViZm12cGQ0M3JhZjZpeHFxOSZlcD12MV9pbnRlcm5hbF9naWZfYnl_aWQmY3Q9cw/aEWwGhZpWE7AGekRiZ/giphy.gif" width="230"> <!-- Updated URL and width -->
    </div>
...
if st.session_state.section == "dataset":
    ...
    if analysis_type == "Univariate Analysis":
        ...
        # Numerical columns distribution with larger layout
        ...
        if numerical_cols:
            ...
            ax.hist(filtered_df[selected_num_col].dropna(), bins=30, color='#86A8E7', edgecolor='black')
            ax.set_title(f'Distribution of {selected_num_col.replace("_", " ").title()}')
            ax.set_xlabel(selected_num_col.replace("_", " ").title())
        ...
        # Filter for object (categorical) columns only, excluding poster_link, series_title, overview
        ...
        if categorical_cols:
            ...
            ax.barh(y_pos, counts.values, color='#86A8E7')
            ax.set_yticks(y_pos)
            ax.set_yticklabels(counts.index)
            ax.set_xlabel('Count')
            ax.set_ylabel(selected_cat_col.title())
            ax.set_title(f'Top 10 {selected_cat_col.replace("_", " ").title()}')
            plt.tight_layout()
            st.pyplot(fig)
        else:
        ...
        if selected_bivariate == "Gross by Certificate" and 'gross' in filtered_df.columns and 'certificate' in filtered_df.columns:
            avg_gross = filtered_df.groupby('certificate')['gross'].mean().sort_values(ascending=False).head(10)

            fig, ax = plt.subplots(figsize=(10, 6))
            y_pos = np.arange(len(avg_gross))
            ax.barh(y_pos, avg_gross.values, color='#86A8E7')
            ax.set_yticks(y_pos)
            ax.set_yticklabels(avg_gross.index)
            ...
            avg_gross = filtered_df.groupby('primary_genre')['gross'].mean().sort_values(ascending=False).head(10)
            ...
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.scatter(filtered_df['imdb_rating'], filtered_df['gross'], alpha=0.6, color='#86A8E7')
            ax.set_xlabel('IMDB Rating')
            ax.set_ylabel('Gross Revenue ($)')

            # Add trendline
            z = np.polyfit(filtered_df['imdb_rating'].dropna(), filtered_df['gross'].dropna(), 1)
            p = np.poly1d(z)
            # Ensure the x-values for the trendline match the scatter plot's x-axis range
            x_values = np.linspace(filtered_df['imdb_rating'].min(skipna=True), filtered_df['imdb_rating'].max(skipna=True), 100)
            ax.plot(x_values, p(x_values), color='#ff7e5f', linewidth=2, alpha=0.8)
            ...
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.scatter(filtered_df['no_of_votes'], filtered_df['gross'], alpha=0.6, color='#86A8E7')
            ax.set_xlabel('Number of Votes')
            ax.set_ylabel('Gross Revenue ($)')

            # Add trendline
            z = np.polyfit(filtered_df['no_of_votes'].dropna(), filtered_df['gross'].dropna(), 1)
            p = np.poly1d(z)
             # Ensure the x-values for the trendline match the scatter plot's x-axis range
            x_values = np.linspace(filtered_df['no_of_votes'].min(skipna=True), filtered_df['no_of_votes'].max(skipna=True), 100)
            ax.plot(x_values, p(x_values), color='#ff7e5f', linewidth=2, alpha=0.8)
            ...
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.scatter(filtered_df['runtime'], filtered_df['imdb_rating'], alpha=0.6, color='#86A8E7')
            ax.set_xlabel('Runtime (minutes)')
            ax.set_ylabel('IMDB Rating')

            # Add trendline
            z = np.polyfit(filtered_df['runtime'].dropna(), filtered_df['imdb_rating'].dropna(), 1)
            p = np.poly1d(z)
             # Ensure the x-values for the trendline match the scatter plot's x-axis range
            x_values = np.linspace(filtered_df['runtime'].min(skipna=True), filtered_df['runtime'].max(skipna=True), 100)
            ax.plot(x_values, p(x_values), color='#ff7e5f', linewidth=2, alpha=0.8)
            ...
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.scatter(filtered_df['meta_score'], filtered_df['imdb_rating'], alpha=0.6, color='#86A8E7')
            ax.set_xlabel('Meta Score')
            ax.set_ylabel('IMDB Rating')

            # Add trendline
            z = np.polyfit(filtered_df['meta_score'].dropna(), filtered_df['imdb_rating'].dropna(), 1)
            p = np.poly1d(z)
            # Ensure the x-values for the trendline match the scatter plot's x-axis range
            x_values = np.linspace(filtered_df['meta_score'].min(skipna=True), filtered_df['meta_score'].max(skipna=True), 100)
            ax.plot(x_values, p(x_values), color='#ff7e5f', linewidth=2, alpha=0.8)
    ...
    if analysis_type == "Top/Bottom Movies by Gross" and 'gross' in filtered_df.columns:
        ...
        if selection_type == "Top 5 Movies":
            ...
            ax.barh(y_pos, top_5['gross'], color='#86A8E7')
            ax.set_yticks(y_pos)
            ax.set_yticklabels(top_5['series_title'])
            ...
            ax.barh(y_pos, bottom_5['gross'], color='#ff7e5f')
            ax.set_yticks(y_pos)
            ax.set_yticklabels(bottom_5['series_title'])
            ax.set_xlabel('Gross Revenue ($)')
        ...
        if 'gross' in filtered_df.columns and 'is_sequel' in filtered_df.columns:
            ...
            with col1:
                ...
                colors = ['#ff7e5f', '#86A8E7']
                ax.bar(sequel_comparison['is_sequel'], sequel_comparison['gross'], color=colors)
            ...
            with col2:
                ...
                fig, ax = plt.subplots(figsize=(8, 6))
                colors = ['#86A8E7', '#ff7e5f']  # Same colors as bar chart
                wedges, texts, autotexts = ax.pie(sequel_count.values, labels=['Standalone', 'Sequels'], autopct='%1.1f%%',
