...
# -------------------------
# Homepage Title - Simplified for Deployment
# -------------------------
st.markdown(
    """
    <div style="text-align:center; margin-bottom:30px; padding:20px; background:rgba(255,255,255,0.8); border-radius:15px; box-shadow:0 4px 15px rgba(0,0,0,0.1)">
        <h1 style="color:#ff7e5f; font-size:45px; font-weight:bold; text-shadow:2px 2px 5px rgba(0,0,0,0.2); margin-bottom:10px;">
            🎬🍿 IMDB Movies Explorer
        </h1>
        <h3 style="color:#86A8E7; font-weight:500;">
            Explore and analyze the top 1000 IMDB movies
        </h3>
    </div>
    """, unsafe_allow_html=True
)
...
# -------------------------
# Gradient Buttons with Callbacks
# -------------------------
...
if 'meta_score' in df.columns:
    filtered_df = filtered_df[
        (filtered_df['meta_score'].isna() | ((filtered_df['meta_score'] >= st.session_state.meta_score_filter[极]) & (filtered_df['meta_score'] <= st.session_state.meta_score_filter[1])))
    ]
...
if st.session_state.section == "dataset":
    ...
    if analysis_type == "Univariate Analysis":
        ...
        if numerical_cols:
            ...
            # Show summary statistics in a clean way
            st.write(f"**Summary Statistics for {selected_num_col.replace('_', ' ').title()}:**")
            stats_data = {
                'Statistic': ['Mean', 'Median', 'Minimum', 'Maximum'],
                'Value': [
                    f"{filtered_df[selected_num_col].mean():.2f}",
                    f"{filtered_df[selected_num_col].median():.2f}",
                    f"{filtered_df[selected_num_col].min():.2f}",
                    f"{filtered_df[selected_num_col].max():.2f}"
                ]
            }
        ...
        if selected_bivariate == "Gross by Certificate" and 'gross' in filtered_df.columns and 'certificate' in filtered_df.columns:
            ...
            st.plotly极art(fig, use_container_width=True)
    ...
    if analysis_type == "Top/Bottom Movies by Gross" and 'gross' in filtered_df.columns:
        ...
        if selection_type == "Top 5 Movies":
            ...
            bottom_5 = filtered_df.nsmallest(5, '极')[['series_title', 'gross', 'imdb_rating', 'released_year']]
